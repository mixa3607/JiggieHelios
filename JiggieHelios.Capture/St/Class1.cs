...
public class BitmapVideoFrameWrapper : IVideoFrame, IDisposable
{
    ...
    public Image<Argb32> Source { get; private set; }

    public BitmapVideoFrameWrapper(Image<Argb32> bitmap)
    {
        Source = bitmap ?? throw new ArgumentNullException(nameof(bitmap));
        Format = ConvertStreamFormat(bitmap.PixelType);
    }
    ...
    {
        if (Source.DangerousTryGetSinglePixelMemory(out var pixels))
        {
            pixels.
            return new DecodedTexture()
            {
                DisposableObject = image,
                Pixels = pixels.Cast<Rgba32, byte>(),
    ...
    private static string ConvertStreamFormat(PixelTypeInfo fmt)
    {
        if (fmt is { BitsPerPixel: 8, AlphaRepresentation: PixelAlphaRepresentation.Associated })
        {
            return "rgba";
        }
