...
public class SkiaRender
{
    ...
    public List<SkiaRenderSet> Sets { get; set; } = new();
    ...
    public void LoadImageSetsFromGameState(GameState state, string imagesDir, int threads = 1)
    {
        ...
        foreach (var set in state.Sets)
        {
            ...
            var renderSet = new SkiaRenderSet() { Bitmap = null, Canvas = null };
            renderSet.Pieces.EnsureCapacity(set.Pieces.Count);

            var tmp = new SkiaRenderSetPiece?[set.Pieces.Count];
            ...
            }, (i) =>
            {
                ...
                tmp[i] = new SkiaRenderSetPiece() { Image = pieceImg };
            });
